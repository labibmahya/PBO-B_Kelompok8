...
class Main(DataManager, perpus.MyFrame1):
    ...
    def button_login( self, event ):
        ...
        self.query = f"SELECT * FROM user where username = '{username}' and password = '{password}'"
...
class Main3(Main2, perpus.MyFrame3):
    def __init__(self, parent, userr):
        ...
        self.userr = userr
        self.tampil()
        self.m_staticText39.SetLabel(userr)
    ...
    def refresh( self, event ):
        event = Main3(None, userr = self.userr)
        event.Show()
        self.Destroy()   

    def btn_deleteBuku( self, event ):
        kodeBuku = self.m_textCtrl18.GetValue()
        self.query = f"DELETE From buku WHERE kodeBuku = '{kodeBuku}'"
        self.DM.Jalankan(self.query)        
        if kodeBuku != "":
            self.refresh(event)
